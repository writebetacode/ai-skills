...
**Authoring violation:** editing a `skills/*/SKILL.md` or an `agents/*/AGENT.md` without loading `skill-write` first, including a file the change only touches in passing and a rule restated from memory of the skill. A one-line fix to a command table, made directly because the edit looked too small to be worth the load, is a violation; loading the skill and then making that same one-line fix is not.
