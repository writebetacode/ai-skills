...
**Authoring violation:** editing any file under `skills/` or `agents/` without loading `skill-write` first, including a file the change only touches in passing and a rule restated from memory of the skill. A one-line fix to a command table in a `github.md`, or a renamed heading in one of the artifact templates `/sdlc-design` carries, made directly because the edit looked too small to be worth the load, is a violation; loading the skill and then making that same one-line fix is not.
