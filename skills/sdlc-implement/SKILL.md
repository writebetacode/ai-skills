...
**Handoff.** Hand each agent the task path plus the project root; they load their own context. Tester and coder run at fixed `high` effort and self-allocate reasoning depth; no per-task depth flag.
