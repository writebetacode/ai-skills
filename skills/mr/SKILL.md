...
Always assign to the current user by resolved username — `glab` has no `@me` equivalent, so resolve it via `glab api user` rather than hardcoding. Always pass `--yes` on create; without it `glab` blocks on an interactive confirmation prompt. Never apply a closing keyword to a Jira key. Use only ASCII; never include AI attribution or "Co-Authored-By" lines.
