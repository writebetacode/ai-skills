...
Resolve the forge from the `origin` remote, then dispatch every remote operation to that host's agent -- `gh` for GitHub, `glab` for GitLab -- via the `Agent` tool, resuming it with `SendMessage` for later operations in the same run. Where a self-hosted URL settles nothing, ask each agent for `repo-id` and take the one that resolves; if both do or neither does, ask the user rather than guessing.

Send `op:` and its parameters one per line, and pass the release notes as a file path -- the bytes never travel as prose in a message. Git itself stays in this skill: tags, log, and remote resolution are local operations with no agent between them.
...
**Close with the changelog range.** End the body with a compare link spanning the previous tag to the new one, matching the form recent releases use -- typically `**Full Changelog**: <compare-url>` as the last line. The previous tag is the one the version was inferred against, so the range covers exactly the commits described above it. Derive the base URL from `git remote get-url origin`, converting an SSH remote (`git@host:owner/repo.git`) to its `https://host/owner/repo` form, then build the path for the host:
...
GitLab puts `/-/` before the route and GitHub does not, so a link built for the wrong forge 404s. On a repo's first release there is no previous tag: omit the link rather than inventing a range.

**Publish on confirmation.** Create an annotated tag (`git tag -a <version> -m <title>`) when the repo's recent tags are annotated, a lightweight tag when they are not; `git cat-file -t "$(git rev-parse <tag>)"` reports `tag` for annotated and `commit` for lightweight. Push it, then write the drafted body to a temp file outside the repo and dispatch `release-create` with the tag, title, and notes path -- plus the target branch on GitHub, which GitLab does not take.

Push the tag before dispatching, always. The two forges fail opposite ways when it is missing, and the agents are configured so neither one hides it: `gh` refuses the create outright, while `glab` would otherwise create the tag itself from a ref and mask the failed push. Ask for GitHub's `--generate-notes` only when the drafted body is meant to carry gh's commit list beneath it; GitLab has no equivalent. Display the release URL from the agent's report.

## Rules

Never publish without an explicit confirmation covering the final version, title, and body together. Never tag from a branch other than the resolved default branch, or from a dirty tree. Never reuse an existing tag -- check with `git rev-parse <version>` first and stop if it resolves. On GitLab this matters more than a convention: creating against a tag that already has a release overwrites its name and notes rather than failing, so the agent pins `--no-update`, and a version that resolves locally is stopped here before it ever reaches that command. Never invent a version that skips or reorders the sequence.
