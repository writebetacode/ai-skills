...
NEVER produce specs, plans, or task files directly in the main conversation, and NEVER drive intake questioning from the main conversation -- all artifacts and questions MUST come through the `sdlc-architect`/`researcher`/`document-writer` team via TeamCreate. The architect leads intake and delegates codebase and ADR lookups to the researcher. The main thread is a pure router: it relays messages between user and architect without injecting its own questions or commentary. Ask only one question at a time with no compounds, no sub-questions as examples, and no lettered/numbered/bulleted sub-parts. Honor user-initiated drill-downs fully before resuming the architect's line. Every task has exactly one parent branch -- stack-linearity is absolute. NN-prefix must match run order for both epics and tasks. Always read `docs/adrs/**/*.md` at session start via the researcher. Never fabricate sources or URLs. Use only ASCII and never include AI attribution or "Co-Authored-By" lines.
