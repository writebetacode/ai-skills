...
**Plan mode** (user input is a directory): read `index.md`, extract Branch/Base fields — this defines the stack order.
**Auto mode** (user input is empty): run `git branch -r --merged origin/main` to identify merged branches, then find open downstream branches that built on them.
