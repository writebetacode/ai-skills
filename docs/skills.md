...
Every mode reviews a checkout, not your working tree. The skill fetches the PR's head ref — `refs/pull/<n>/head` on GitHub, `refs/merge-requests/<iid>/head` on GitLab, both served by the base project, so a fork needs no extra remote — and adds a detached worktree at `/tmp/pr-review-<repo-slug>-<number>`, the slug being what stops two repos reviewing their own PR 22 from colliding on one path. That checkout outlives the run, because a later `post 2 and 5` has to re-read the anchors and quotes it sends; it is reused and moved to the current head rather than duplicated, and removed once nothing is left for a later turn to read it — neither a finding waiting to be posted nor a thread waiting for a reply — or once the run settles that there never will be. A checkout that lives across turns outlives whatever cleans `/tmp`, so a registration left pointing at a directory that is gone is pruned and remade rather than treated as a failure — without that, the second review of a PR dead-ends on git refusing both the reuse and the fresh add. Wherever it is left standing, the path is named in what you get back. Your branch, working tree, and stash are never touched, and no finding can cite a line that only exists in whatever you had checked out at the time. A checkout that fails stops the run rather than falling back to reading the diff alone.
...
That checkout is what the report quotes from. Each finding carries the code it rests on in a fenced block labelled `<file>:<line-range>` — the lines it names, plus the caller, the definition, or the stale test the claim depends on. Where the evidence is an absence, the finding names the search that came back empty. Posted comments carry the off-diff quotes only, since the inline comment already sits on the line it is about. GitHub's batched review is the one place a body does not travel as a file — it goes inside a JSON string — so that payload is assembled with `jq --rawfile` rather than by hand, which is what keeps a fenced quote byte-exact on the way up.

The bar is what the quoted code supports. A claim the code does not back is dropped rather than reworded as a question or shrunk into a smaller label — `nitpick` and `polish` are not offered at all, since a label that exists gets used — and remarks about how the code is written — naming, structure, an idiom someone else would have picked — are not findings at all. A review with nothing to report says so instead of filling the file.

Your repo's own rules move that bar, in both directions. The checkout is read for `CONTRIBUTING.md`, `CLAUDE.md`, `AGENTS.md`, `.cursorrules`, `docs/architecture/`, `docs/adrs/`, `.editorconfig` and whatever linter and formatter configs exist, plus the nearest of those above each changed path so a monorepo package keeps its own rules. A rule the change breaks is a finding quoting that rule, because style stops being taste once you have written it down; a rule that permits what the review was about to flag kills the finding. Silence leaves the behavior-only bar standing, and no convention excuses something that actually breaks. Two things the guidelines cannot do: rules your linter or formatter already enforces are left to CI rather than restated in a comment, and a guidelines file the PR itself edits is reviewed as part of the change instead of being obeyed — otherwise a fork could relax the rules it is about to be judged by.
