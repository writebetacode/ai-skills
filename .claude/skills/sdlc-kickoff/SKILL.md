---
name: sdlc-kickoff
description: Spin up a Claude Code agent team that researches a topic, decomposes findings into independent epics, brainstorms each epic in parallel, then validates all output before handing off to sdlc-plan.
---
...
Spawn a single decomposition teammate with model `opus`. Its prompt must instruct it to read all research output from phase 1 and identify every self-contained, dependency-free feature area -- treating each as an epic. An epic is self-contained if it can be fully specified and implemented without requiring another epic to be completed first. The agent must produce `plans/kickoff-YYYY-MM-DD/epics.md` containing a numbered list of epics, each with a short title, a 2-3 sentence description of its scope, and the relevant research findings that informed it. Wait for this agent to go idle before proceeding. The epic list in this file drives all of phase 3.
...
Each pair of brainstorm agents is assigned one epic from `epics.md`. Within each pair, one agent focuses on core feature definition for that epic and the other focuses on edge cases, alternative approaches, and integration concerns. Each brainstorm agent's prompt must instruct them to invoke the `sdlc-brainstorm` skill using the Skill tool, passing their epic description and the relevant phase 1 `ai-summary.md` content as seed context. When the skill raises a question that cannot be answered from context, the agent must send it to the research liaison via team messaging and wait for the response before continuing. Each spec must be written to `plans/YYYY-MM-DD-<slug>/spec.md` exactly as the brainstorm skill requires, using the epic title as the slug. Wait for all brainstorm agents and the liaison to go idle before proceeding.
...
I would like a sdlc lifecycle skill that can be invoked to spin up a team of agents that will follow the research skill and then subsequently perform the brainstorm skill to build out features that are required from the initial research. the research and brainstorm will ask and answer all of these themselves and create the files needed after the brainstorm so that I can review them and generate the plans for them myself. I tried doing this myself, however it just ignored the skills, which is something that I don't want at all. also the agents spun up should be ran with all permissions to limit those issues
