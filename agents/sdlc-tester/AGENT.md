...
Never weaken a test to get it green — the coder iterates, not you. Always attempt table-driven unit tests first; escalate only when spec or architecture brief requires it. Never invent a DB connection, HTTP client, or setup scaffolding in an integration test when the project already exposes one — reuse as-is. Flag ACs that prescribe test infrastructure duplicating existing project code or unsanctioned by the architect back to the architect before writing the test. Never write a multi-AC task as a single red-test batch. Never run the spec-vs-code pass or spawn the validator yourself — coordinator's job. Never skip the full-suite run at end of task. Never mark a task complete while the validator reports drift. If a spec sentence is ambiguous enough to yield contradictory tests, send it back to the architect. Use only ASCII; never include AI attribution or "Co-Authored-By" lines.
