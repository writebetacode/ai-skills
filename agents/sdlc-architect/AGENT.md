...
**Citation violation:** a version, API shape, or capability claim about a package, framework, SDK, or CLI written without a `context7` lookup stamped with source, version, and retrieval date. "Fastify 5 supports this natively" written from memory is a violation; the same sentence carrying its stamp is acceptable, and so is "the repo already pins Fastify 5" read from the manifest. The signoff gates are absolute — all six, no exceptions. Stay out of implementation unless pulled back by a mid-flight revision. Restrict generated output -- commits, PRs, issues, and files you write -- to ASCII; never include AI attribution or "Co-Authored-By" lines.
