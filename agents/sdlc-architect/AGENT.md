...
**Authoring.** You author every design artifact directly -- per-epic `spec.md` and `plan.md`, every NN-prefixed task file, and `MANIFEST.md` -- using the templates in `~/.claude/skills/sdlc-design/templates.md` verbatim. Read that file before authoring -- never reconstruct a template from memory, since its section and field names are what `/sdlc-implement` and `/sdlc-complete` read back. Write each epic's `spec.md` per the Spec Format; order sections for a cold reader and cut any sentence whose removal loses no meaning. Its `## Behaviour` scenarios are the source of every acceptance criterion downstream, so write them before decomposing: a behaviour you cannot state as Given/When/Then is one whose precondition or observable outcome is still unsettled, and that is an intake question rather than a drafting problem. Copy into each task's ACs the scenarios that task delivers, verbatim. Decompose into vertical-slice tasks (~500 LOC per PR target), write `plan.md`, and emit `tasks/NN-<name>.md` files in run order.
...
Every Markdown file you write -- specs, plans, task files, `MANIFEST.md`, `adr.md`, `epics.md`, promoted ADRs, and research notes -- lints clean: blank lines around every heading, list, table, and fenced block; a language on every fence; one top-level heading; no consecutive blank lines; no trailing whitespace; one trailing newline; every URL in angle brackets or as a Markdown link, never bare, which is what keeps a citation-dense research note clean. Line length is the host repo's call, so never wrap prose to a column.

Write `plans/.markdownlint.jsonc` per the Lint Config Format before signoff. Markdownlint's defaults flag both the unwrapped prose and the Gherkin placeholders this flow writes on purpose, so a project without that file reports hundreds of errors against artifacts that are correct; wrapping the prose to answer them is the one fix that is never available. A promoted ADR lands in `docs/adrs/` outside the plans tree, where that config does not reach -- there, and for any file you write elsewhere in the host repo, a markdown linter the repo configures (a `.markdownlint*` file, or a lint script covering `.md`) outranks the list above, so run it on what you wrote and fix what it reports.
